   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : 93730285765")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😘\nFor example: +93730285765 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
//------------------------------------------------------
    XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
           ...
           await delay(1000 * 2);
           const xeonses = await XeonBotInc.sendMessage(XeonBotInc.user.id, { text: sessionXeon });
             await XeonBotInc.sendMessage(XeonBotInc.user.id, { text: `*SESSION ID GENERATED SUCCESSFULY* ✅
